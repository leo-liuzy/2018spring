...
SMALLEST_POS = sys.float_info.min

use_cuda = torch.cuda.is_available()


class MMIBeam(object):
    """
    ...
    def __init__(self,
        ...
        # self.sequences = self.sequences.cuda() if use_cuda else self.sequences
        ...
        # self.seq_log_prob = self.seq_log_prob.cuda() if use_cuda else self.seq_log_prob

        self.output_hiddens = Variable(torch.zeros(self._batch_size, self._beam_width, self._decoder_output_dim))
        # self.output_hiddens = self.output_hiddens.cuda() if use_cuda else self.output_hiddens

        self.output_contexts = Variable(torch.zeros(self._batch_size, self._beam_width, self._decoder_output_dim))
        # self.output_contexts = self.output_contexts.cuda() if use_cuda else self.output_contexts

        self.language_hiddens = Variable(torch.zeros(self._batch_size, self._beam_width, self._decoder_output_dim))
        # self.language_hiddens = self.language_hiddens.cuda() if use_cuda else self.language_hiddens

        self.language_contexts = Variable(torch.zeros(self._batch_size, self._beam_width, self._decoder_output_dim))
        # self.language_contexts = self.language_contexts.cuda() if use_cuda else self.language_contexts
    ...
    def batched_index_select(self, batched_input, batched_indices: torch.LongTensor):
        ...
        return torch.stack(result, 0) # .cuda() if use_cuda else torch.stack(result, 0)
    ...
    def fold(self, class_probabilities, num_classes):
        ...
        stop_tensor = Variable(torch.LongTensor(self._beam_width, 1).fill_(self._end_index)).cpu()
        one_tensor = Variable(torch.ByteTensor(self._beam_width, 1).fill_(1)).cpu()
        ...
        for i in range(self._batch_size):
            ...
            while not stop:
                ...
                log_prob_cur, indices = torch.topk(cur_batch.view(-1, 1), self._beam_width, dim=0)
                ...
                if torch.equal(unfinished_tensor, one_tensor):
                    ...
                    finished_index = torch.masked_select(indices, finished_tensor) / num_classes
                    ...
                    for index in finished_index:
                        ...
                        cur_batch_next[nth_beam, self._end_index] = -sys.float_info.max
    ...
    def update(self, output_projections, hiddens, contexts,
        ...
        anti_probabilities = F.softmax(lm_projections, dim=-1)
        assert class_probabilities.size() == anti_probabilities.size()

        anti_probabilities = anti_probabilities * g_k
        # (batch, beam_width, num_classes)
        combined_probabilities = class_probabilities - self._lambd * anti_probabilities

        update_indices, update_log_prob = self.fold(combined_probabilities, num_classes)
        ...
        self.output_hiddens = torch.index_select(hiddens[0], 0, nth_beam.view(-1))
        self.output_contexts = torch.index_select(contexts[0], 0, nth_beam.view(-1))

        self.language_hiddens = torch.index_select(lm_hiddens[0], 0, nth_beam.view(-1))
        self.language_contexts = torch.index_select(lm_contexts[0], 0, nth_beam.view(-1))
